...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("uncond_image", nn.Parameter(torch.zeros((4,64,64)))) #if image size is 512


    def sample_drop(self, z_src, force_drop_ids=None):
        '''
        z_src : (B, 4, 32, 32)
        returns z_src with some of the samples replaced with 0s 
        '''
        drop_ids = torch.rand(z_src.shape[0]).cuda() < self.dropout_prob
        drop_ids = torch.logical_or(drop_ids, force_drop_ids)
        z_src = torch.where(drop_ids[:, None, None, None], self.uncond_image, z_src)
        return z_src

    def forward(self, x, z_src, train, force_drop_ids=None):
        if (train and self.dropout_prob>0) or (force_drop_ids is not None):
            z_src = self.sample_drop(z_src, force_drop_ids)
        x = torch.cat([x,z_src], axis=1) #(bs, 8, 64, 64)
        x = self.proj(x) #(bs, 1152, 64, 64)
        if self.flatten:
...
class PixArtMS(PixArt):
    ...
    def __init__(
        ...
        print("GOT INPUT SIZE: ", input_size)
        super().__init__(
        ...
        self.x_embedder = PatchEmbed(patch_size, in_channels, hidden_size, bias=True, dropout_prob = class_dropout_prob)
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        ...
        both_uncond_prob = max(self.x_embedder.dropout_prob, self.y_embedder.uncond_prob)
        if self.training:
            assert y.shape[0] == z_src.shape[0]
            force_drop_ids = torch.rand(y.shape[0]).cuda() < both_uncond_prob
        else:
            force_drop_ids = None


        x = self.x_embedder(x, z_src, self.training, force_drop_ids) + pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
...
def PixArtMS_XL_2(**kwargs):
    return PixArtMS(depth=28, hidden_size=1152, patch_size=2, num_heads=16, in_channels=8, **kwargs)
