...
def load_checkpoint(checkpoint,
                    ...
                    max_length=120,
                    resuming_editing_checkpoint=False
                    ):
    ...
    if not resuming_editing_checkpoint:
        '''
        Logic to enable loading of weights that've been modified for editing
        '''
        og_w = state_dict['x_embedder.proj.weight']
        print(og_w.shape)
        expanded_x_emb_w = torch.zeros((1152,8,2,2), dtype=og_w.dtype, device=og_w.device)
        expanded_x_emb_w[:,:4, :,:] = og_w
        state_dict['x_embedder.proj.weight'] = expanded_x_emb_w


        # og_w = state_dict['final_layer.linear.weight']
        # print(og_w.shape)
        # expanded_x_emb_w = torch.zeros((64, 1152), dtype=og_w.dtype, device=og_w.device)
        # expanded_x_emb_w[:32,:] = og_w
        # state_dict['final_layer.linear.weight'] = expanded_x_emb_w

        # og_w = state_dict['final_layer.linear.bias']
        # print(og_w.shape)
        # expanded_x_emb_w = torch.zeros((64), dtype=og_w.dtype, device=og_w.device)
        # expanded_x_emb_w[:32] = og_w
        # state_dict['final_layer.linear.bias'] = expanded_x_emb_w
