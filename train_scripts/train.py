...
def log_validation(model, step, device, vae=None):
    ...
    for image_path, prompt in zip(validation_image_paths, validation_prompts):
        ...
        src_image = default_loader(image_path)
        src_image = transform(src_image)
        src_images.append(src_image)

        #according to IP2P codebase the concat representation is the mode of the latent_distbn
        ...
        # print(z.shape, latent_src.shape)
    ...
    torch.cuda.empty_cache()    
    for prompt, latent, og_image in zip(validation_prompts, latents, src_images):
        ...
        og_image = torch.clamp(127.5 * og_image + 128.0, 0, 255).permute(1,2,0).to("cpu", dtype=torch.uint8).numpy()
        image_logs.append({"validation_prompt": prompt, "images": [image], "src_images": [og_image]})
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            for log in image_logs:
                ...
                for og_image, image in zip(src_images, images):
                    combined = np.vstack((og_image, image))
                    image = wandb.Image(combined, caption=validation_prompt)
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if load_vae_feat:
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=(config.mixed_precision == 'fp16' or config.mixed_precision == 'bf16')):
                        posterior_edit = vae.encode(batch[1]).latent_dist
                        z_src = vae.encode(batch[0]).latent_dist.mode()
                        if config.sample_posterior:
                            z_edit = posterior_edit.sample()
                        else:
                            z_edit = posterior_edit.mode()

            clean_images = z_edit * config.scale_factor
            data_info = batch[4]
            ...
            if load_t5_feat:
                ...
                with torch.no_grad():
                    txt_tokens = tokenizer(
                        batch[2], max_length=max_length, padding="max_length", truncation=True, return_tensors="pt"
            ...
            with accelerator.accumulate(model):
                ...
                optimizer.zero_grad()
                loss_term = train_diffusion.training_losses(model, clean_images, timesteps, model_kwargs=dict(y=y, mask=y_mask, data_info=data_info, z_src=z_src))
...
if __name__ == '__main__':
    ...
    if config.visualize:
        ...
        validation_image_paths = [os.path.join(config.validation_img_dir, k) for k in config.validation_images]
        skip = True
    ...
    if config.load_from is not None:
        missing, unexpected = load_checkpoint(
            config.load_from, model, load_ema=config.get('load_ema', False), max_length=max_length, optimizer=None, lr_scheduler=None, resume_optimizer=False,
            resume_lr_scheduler=False,
            resuming_editing_checkpoint=config.loading_editing_checkpoint)
    ...
    if config.resume_from is not None and config.resume_from['checkpoint'] is not None:
        ...
        _, missing, unexpected = load_checkpoint(**config.resume_from,
                                                 ...
                                                 resuming_editing_checkpoint=True
                                                 )
